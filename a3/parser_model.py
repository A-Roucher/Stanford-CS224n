...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.hidden_to_logit_weight = nn.Parameter(
            nn.init.xavier_uniform_(
                torch.zeros((self.hidden_size, self.n_classes))
            )
        )
        self.hidden_to_logit_bias = nn.Parameter(
            nn.init.uniform_(
    ...
    def forward(self, w):
        ...
        bias_for_hidden = self.embed_to_hidden_bias * torch.ones(batch_size, self.hidden_size)
        hidden = nn.functional.relu(self.embed_to_hidden_weight.T @ e.T + bias_for_hidden.T)
    
        bias_for_logit = self.hidden_to_logit_bias * torch.ones(batch_size, self.n_classes)
        logits = self.hidden_to_logit_weight.T @ hidden + bias_for_logit.T
        return logits.T
