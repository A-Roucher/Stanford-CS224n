...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            self.stack = [self.stack[i] for i in range(len(self.stack)) if i!=len(self.stack)-2]

        elif transition == 'RA':
            self.dependencies.append((self.stack[-2], self.stack[-1]))
            self.stack = [self.stack[i] for i in range(len(self.stack)) if i!=len(self.stack)-1]
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        to_remove = []
        for ix, partial_parse in enumerate(minibatch):
            transition = model.predict([partial_parse])
            partial_parse.parse_step(transition[0])
            if len(partial_parse.buffer) == 0 and len(partial_parse.stack) == 1:
                to_remove.append(ix)

        unfinished_parses = [unfinished_parses[i] for i in range(len(unfinished_parses)) if i not in to_remove]
